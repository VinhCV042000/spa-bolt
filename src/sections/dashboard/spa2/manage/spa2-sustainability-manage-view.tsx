...
export function Spa2SustainabilityManageView() {
  ...
  const reorderStats = (next: Spa2SustainabilityStat[]) => {
    setStats(next);
    markDirty();
  };
  ...
  const reorderCommitments = (next: Spa2SustainabilityCommitment[]) => {
    setCommitments(next);
    markDirty();
  };
  ...
  const reorderMilestones = (next: Spa2SustainabilityMilestone[]) => {
    setMilestones(next);
    markDirty();
  };
    ...
    >
      ...
      {tab === 'commitments' && (
        <Card sx={{ p: 3, borderRadius: 3 }}>
          ...
          <Spa2SortableGrid items={commitments} onReorder={reorderCommitments}>
            <Grid container spacing={2}>
        ...
        <Grid container spacing={3}>
          <Grid xs={12} md={6}>
            ...
            >
              ...
              <Spa2SortableGrid items={milestones} onReorder={reorderMilestones}>
                <Stack spacing={2}>
                  {milestones.map((item) => (

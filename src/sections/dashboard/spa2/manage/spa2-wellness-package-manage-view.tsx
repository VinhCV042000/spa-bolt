...
export function Spa2WellnessPackageManageView() {
  ...
  const reorderPackages = (next: Spa2WellnessPackageItem[]) => {
    setPackages((prev) => {
      if (tagFilter === 'all') return next;
      const queue = [...next];
      return prev.map((p) => (p.tag === tagFilter ? (queue.shift() as Spa2WellnessPackageItem) : p));
    });
    markDirty();
  };
  ...
  const reorderAddons = (next: Spa2WellnessAddon[]) => {
    setAddons(next);
    markDirty();
  };
    ...
    >
      ...
      {tab === 'addons' && (
        <Card sx={{ p: 3, borderRadius: 3 }}>
          ...
          <Spa2SortableGrid items={addons} onReorder={reorderAddons}>
                    ...
                    {(sortable) => (
                      <Box sx={{ position: 'relative' }}>
                        ...
                        >
                          <Spa2DragHandle
                            sortable={sortable}
                            sx={{ bgcolor: 'common.white', boxShadow: 1, p: 0.5 }}
                          />
                          <IconButton
                            size="small"
